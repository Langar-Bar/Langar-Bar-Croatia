...
create table if not exists public.opening_management (
 ...
 status text not null default 'opening_soon' check(status in ('opening_soon','soft_opening','grand_opening','open_now')),
...
begin
 ...
 update public.opening_management set
  ...
  headline_en=coalesce(p_settings->>'headline_en',headline_en),headline_hr=coalesce(p_settings->>'headline_hr',headline_hr),
  announcement_en=coalesce(p_settings->>'announcement_en',announcement_en),announcement_hr=coalesce(p_settings->>'announcement_hr',announcement_hr),
...
commit;
select * from public.opening_management;
