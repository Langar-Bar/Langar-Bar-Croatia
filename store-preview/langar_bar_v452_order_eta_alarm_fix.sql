-- Langar Bar V4.5.2 — Order ETA, customer status and louder admin alarm support
-- Run this once in Supabase SQL Editor after uploading V4.5.2.
...
begin
  ...
  insert into public.customer_orders (
    ...
    false,
    now(),
    jsonb_build_array(jsonb_build_object('status','new','at',now(),'by','customer_app','version','v452'))
  )
...
$$;
grant execute on function public.admin_update_customer_order(uuid,text,boolean,text,integer,text) to authenticated;
