-- Langar Bar V4.5.3 — Admin Cloud Health, custom ETA, order filters and test cleanup
-- Run this once in Supabase SQL Editor after uploading V4.5.3.
-- This version fixes guest/dine-in order submission, customer order tracking by token,
-- admin tablet order read/update through secure RPC functions, and estimated ready-time updates for customers.
...
drop function if exists public.admin_update_customer_order(uuid,text,boolean,text,integer,text);
drop function if exists public.admin_update_customer_order(uuid,text,boolean,text,integer,text,boolean);
drop function if exists public.admin_delete_test_order(uuid);
drop function if exists public.admin_delete_test_orders();
drop function if exists public.admin_cloud_health();
...
begin
  ...
  insert into public.customer_orders (
    ...
    false,
    now(),
    jsonb_build_array(jsonb_build_object('status','new','at',now(),'by','customer_app','version','v453'))
  )
...
begin
  ...
  if p_estimated_minutes is not null and (p_estimated_minutes < 0 or p_estimated_minutes > 240) then
    raise exception 'Estimated minutes must be between 0 and 240';
  end if;

  v_eta_changed := (p_estimated_minutes is not null) or (v_note is not null);
  ...
  set
    ...
    is_test = coalesce(p_is_test, co.is_test),
    updated_at = v_now,
    status_updated_at = case when (v_status is not null and v_status is distinct from v_old.status) or v_eta_changed then v_now else co.status_updated_at end,
    ...
    status_history = case when (v_status is not null and v_status is distinct from v_old.status) or v_eta_changed then
      coalesce(co.status_history,'[]'::jsonb) || jsonb_build_array(jsonb_build_object(
        'from',v_old.status,
        'to',coalesce(v_status,v_old.status),
...
$$;
grant execute on function public.admin_update_customer_order(uuid,text,boolean,text,integer,text,boolean) to authenticated;



-- Admin-only cloud health check for the tablet/admin panel.
create or replace function public.admin_cloud_health()
returns table (
  check_name text,
  ok boolean,
  count_value bigint,
...
begin
  ...
  if to_regclass('public.profiles') is not null then
    execute 'select count(*)::bigint, max(created_at)::timestamptz from public.profiles' into v_count, v_last;
    check_name := 'Profiles'; ok := true; count_value := coalesce(v_count,0); last_at := v_last; message := 'Customer profiles stored in Cloud'; return next;
  else
    check_name := 'Profiles'; ok := false; count_value := 0; last_at := null; message := 'profiles table missing'; return next;
  end if;

  if to_regclass('public.reward_cards') is not null then
    execute 'select count(*)::bigint, max(created_at)::timestamptz from public.reward_cards' into v_count, v_last;
    check_name := 'Rewards'; ok := true; count_value := coalesce(v_count,0); last_at := v_last; message := 'Reward cards stored in Cloud'; return next;
  else
    check_name := 'Rewards'; ok := false; count_value := 0; last_at := null; message := 'reward_cards table missing'; return next;
  end if;

  if to_regclass('public.inbox_messages') is not null then
    execute 'select count(*)::bigint, max(created_at)::timestamptz from public.inbox_messages' into v_count, v_last;
    check_name := 'Inbox'; ok := true; count_value := coalesce(v_count,0); last_at := v_last; message := 'Cloud inbox messages available'; return next;
  else
    check_name := 'Inbox'; ok := false; count_value := 0; last_at := null; message := 'inbox_messages table missing'; return next;
  end if;

  execute 'select count(*)::bigint, max(created_at)::timestamptz from public.customer_orders' into v_count, v_last;
  check_name := 'Orders'; ok := true; count_value := coalesce(v_count,0); last_at := v_last; message := 'Customer orders stored in Cloud'; return next;

  execute 'select count(*)::bigint, max(created_at)::timestamptz from public.admin_members where active=true' into v_count, v_last;
  check_name := 'Admins'; ok := coalesce(v_count,0) > 0; count_value := coalesce(v_count,0); last_at := v_last; message := 'Active Cloud Admin members'; return next;
end;
$$;
grant execute on function public.admin_cloud_health() to authenticated;

-- Admin-only deletion for orders explicitly marked as TEST.
...
begin
  ...
  delete from public.customer_orders where id = p_order_id and is_test = true;
  return found;
end;
$$;
grant execute on function public.admin_delete_test_order(uuid) to authenticated;

create or replace function public.admin_delete_test_orders()
returns integer
language plpgsql
security definer
set search_path = public
...
begin
  ...
  delete from public.customer_orders where is_test = true;
  get diagnostics v_count = row_count;
  return v_count;
end;
$$;
grant execute on function public.admin_delete_test_orders() to authenticated;
...
create index if not exists customer_orders_is_test_idx on public.customer_orders(is_test);
