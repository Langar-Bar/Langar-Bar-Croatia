// Langar Admin V4.3.1 — clean mobile dashboard + English only + responsive panels
(function(){
  'use strict';
  ...
  function installPullRefresh(){
    if(window.__adminPullRefresh431) return; window.__adminPullRefresh431=true;
  ...
  function installSwipe(){
    if(window.__adminSwipe431) return; window.__adminSwipe431=true;
    let sx=0,sy=0;
    document.addEventListener('touchstart',e=>{const t=e.touches[0]; sx=t.clientX; sy=t.clientY;},{passive:true});
    document.addEventListener('touchend',e=>{const t=e.changedTouches[0]; if(!t) return; const target=e.target; if(target && target.closest && target.closest('.table-wrap,.table,.form-card,.legal-block,.cloud-customer-detail,.cloud-customer-list,input,textarea,select,button,a')) return; const dx=t.clientX-sx, dy=t.clientY-sy; if(Math.abs(dx)>120 && Math.abs(dx)>Math.abs(dy)*1.8){ if(dx<0) nextPanel(1); else nextPanel(-1); }},{passive:true});
  }
  ...
  function install(){
    ...
    const obs=new MutationObserver(()=>{ if(isUnlocked() && !window.__admin431Shown){ window.__admin431Shown=true; setTimeout(ensureCleanDashboard,80); } if(!isUnlocked()) window.__admin431Shown=false; });
