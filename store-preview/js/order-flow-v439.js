// Langar App V4.3.9 — customer order accept timer + feedback
(function(){
  'use strict';
  ...
  function currentCart(){ const st=getState(); if(st&&Array.isArray(st.cart)&&st.cart.length) return st.cart; const backup=LS.get('langar_cart_backup_v439',LS.get('langar_cart_backup_v438',[])); return Array.isArray(backup)?backup:[]; }
  function backupCart(){ const st=getState(); if(st&&Array.isArray(st.cart)) LS.set('langar_cart_backup_v439',st.cart); }
  function clearCart(){ const st=getState(); if(st&&Array.isArray(st.cart)) st.cart=[]; LS.set('langar_cart_backup_v439',[]); LS.set('langar_cart_backup_v438',[]); try{ if(window.renderCart) window.renderCart(); }catch{} }
  ...
  function ensureCustomerOrdersBox(){ if($('#customerOrderStatus')) return; const submit=$('#submitOrder'); const cart=submit?.closest('.cart'); if(!submit||!cart) return; const box=document.createElement('section'); box.id='customerOrderStatus'; box.className='order-status-box'; cart.appendChild(box); }
  function decorateOrder(){
    ensurePaymentField();
    $$('.order-mode-segment [data-order-type], .segmented [data-order-type]').forEach(b=>{ if(!b.dataset.v439Mode){ b.dataset.v439Mode='1'; b.addEventListener('click',()=>setOrderType(b.dataset.orderType)); } });
  ...
  function broadcastOrder(order){ try{localStorage.setItem('langar_order_signal_v439',JSON.stringify({id:order.id,at:Date.now(),type:order.type,total:order.total})); localStorage.setItem('langar_order_signal_v438',JSON.stringify({id:order.id,at:Date.now()}));}catch{} try{ if('BroadcastChannel'in window){const bc=new BroadcastChannel('langar_orders'); bc.postMessage({type:'new_order',order}); setTimeout(()=>bc.close(),500);} }catch{} }
  function buildOrder(){
  ...
  function installSubmit(){ const btn=$('#submitOrder'); if(!btn) return; btn.onclick=null; btn.dataset.v439Submit='1'; btn.addEventListener('click',handleSubmit,true); }
  ...
  async function renderCustomerOrders(){ const box=$('#customerOrderStatus'); if(!box) return; const local=localOrdersForCustomer(); paintCustomerOrders(box,local.slice(0,8)); try{ const cloud=await fetchCloudOrders(); paintCustomerOrders(box,mergeOrders(cloud,local).slice(0,8)); }catch{} }
  ...
  async function customerDecision(id,decision){
    ...
    const co=await fetchOneCloudOrder(id); if(co){ let note=appendMarker(co.customer_note||'','CUSTOMER_DECISION',decision); const patch={customer_note:note,updated_at:now,status:decision==='accepted'?'accepted':'cancelled'}; if(decision==='accepted') patch.customer_note=appendMarker(note,'ETA_ACCEPTED_AT',now); await updateCloudOrder(id,patch); did=true; }
    try{ localStorage.setItem('langar_order_signal_v439',JSON.stringify({id,at:Date.now(),decision})); }catch{}
