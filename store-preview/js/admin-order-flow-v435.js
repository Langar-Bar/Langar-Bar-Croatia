...
(function(){
  ...
  function orderTypeLabel(type){ return type==='delivery'?'Delivery':(type==='dine_in'?'Dine-in':'Pick-up'); }
  ...
  function orderCard(o){
    ...
    const payment=meta.paymentMethod==='card_on_delivery'?'Card on delivery / pickup':'Cash';
    return `<article class="admin-order-card ${esc(o.status)}"><header><div><b>${esc(o.id)}</b><small>${new Date(o.createdAt||Date.now()).toLocaleString()} · ${esc(orderTypeLabel(o.type||'pickup'))} · ${o.cloud?'<b>Cloud</b>':'local'}</small></div><span class="tag">${esc(label(o.status,meta))}</span></header>
    <div class="order-admin-grid"><div><h4>Customer</h4><p>${esc(o.name||'-')}<br>${esc(o.phone||'')}${o.type==='delivery'&&o.address?`<br><b>Address:</b> ${esc(o.address)}`:''}${o.type==='dine_in'&&meta.tableNumber?`<br><b>Table:</b> ${esc(meta.tableNumber)}`:''}</p><p><b>Payment:</b> ${payment}<br><b>Total:</b> €${Number(o.total||0).toFixed(2)}</p></div><div><h4>Items</h4><p>${(o.items||[]).map(i=>`${i.qty||i.quantity||1}× ${esc(i.nameSnapshot||i.item_name_en||i.name||'Item')}`).join('<br>')||'<span class="muted">No item rows loaded.</span>'}</p><small>${esc(meta.cleanNote||'')}</small></div></div>
