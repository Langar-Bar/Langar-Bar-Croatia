const CACHE_NAME = 'langar-bar-v4-3-9';
const ASSETS = [
...
self.addEventListener('fetch', event => { if(event.request.method !== 'GET') return; event.respondWith(caches.match(event.request).then(cached => fetch(event.request).then(resp=>resp).catch(()=>cached))); });
