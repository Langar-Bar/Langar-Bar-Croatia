(()=>{
  ...
  const typeLabel=v=>({delivery:'Delivery',pickup:'Pick-up',dine_in:'Dine-in'}[v]||v||'Order');
  ...
  function ticketHtml(o,copyType){
    ...
    const title=delivery?'DELIVERY COPY':'INTERNAL ORDER TICKET';
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>
      ...
      <div class="line"></div><div class="meta"><b>Type:</b> ${esc(typeLabel(o.fulfillment_type||o.type))}<br><b>Created:</b> ${esc(new Date(o.created_at||o.createdAt||Date.now()).toLocaleString())}<br><b>Customer:</b> ${esc(customer)}<br>${phone?`<b>Phone:</b> ${esc(phone)}<br>`:''}${table?`<b>Table:</b> ${esc(table)}<br>`:''}${address?`<b>Address:</b><div class="address">${esc(address)}</div>`:''}</div>
