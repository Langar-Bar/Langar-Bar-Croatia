...
function navigate(id){ $$('.view').forEach(v=>v.classList.toggle('active', v.id===id)); $$('.bottom-nav button').forEach(b=>b.classList.toggle('active', b.dataset.go===id)); if(id==='rewards') renderRewards(); if(id==='referral') renderReferral(); if(id==='gallery') renderGallery(); window.scrollTo({top:0,behavior:'smooth'}); }
function attachNav(){ $$('[data-go]').forEach(b=>{ b.onclick=()=>{ if(b.id==='popupJoin') $('#welcomePopup').classList.add('hidden'); navigate(b.dataset.go); }; }); }
...
function renderDashboard(){ const p=profile(); const d=$('#homeDashboard'); if(!d)return; if(!p){ d.innerHTML=`<div class="dash-card"><h3>${T[state.lang].noProfile}</h3><p>${state.lang==='hr'?'Registrirajte se i primite digitalnu espresso karticu.':'Register and receive your digital espresso card.'}</p><button class="primary" data-go="club">${T[state.lang].join}</button></div>`; attachNav(); return;} const level=p.orders>=30?'VIP':p.orders>=15?'Gold':p.orders>=5?'Silver':'Bronze'; d.innerHTML=`<div class="dash-card"><p class="muted">${T[state.lang].welcome}, ${p.firstName}</p><h3>${p.credit?.toFixed(2)||'0.00'} € Langar Credit</h3><div class="stats"><span>${level}<small>Level</small></span><span>${p.orders||0}<small>Orders</small></span><span>${p.referrals||0}<small>Friends</small></span></div><div class="qr-mini">${p.qr}</div><button class="secondary" data-go="rewards">Rewards</button></div>`; attachNav(); }
...
function renderRewards(){ const p=profile(); const wrap=$('#rewardsView'); if(!wrap)return; if(!p){ wrap.innerHTML='<div class="empty-state">Join Langar Club to unlock rewards.</div>'; return;} const cs=cards(); wrap.innerHTML=`<section class="wallet"><h3>${p.credit?.toFixed(2)||'0.00'} €</h3><p>Langar Credit</p><div class="stats"><span>${p.orders||0}<small>Orders</small></span><span>${p.visits||0}<small>Visits</small></span><span>${p.referrals||0}<small>Referrals</small></span></div></section><h3>Digital Cards</h3><div class="cards-list"></div><h3>History</h3><div class="history-list">${(LS.get('langar_history',[])||[]).map(h=>`<p><b>${h.type}</b> — ${h.text}<br><small>${new Date(h.createdAt).toLocaleString()}</small></p>`).join('')||'<p class="muted">No history yet.</p>'}</div>`; const cl=wrap.querySelector('.cards-list'); cs.forEach(card=>{ const a=document.createElement('article'); a.className='reward-card '+card.status; a.innerHTML=`<h3>${card.title}</h3><p>${card.body}</p><div class="qrbox">${card.code}</div><p>Status: <b>${card.status}</b></p>${card.status==='active'?`<button class="secondary">Redeem / Scan</button>`:''}`; const btn=a.querySelector('button'); if(btn) btn.onclick=()=>redeemCard(card.id); cl.appendChild(a);}); }
function renderReferral(){ const p=profile(); const wrap=$('#referralView'); if(!wrap)return; if(!p){wrap.innerHTML='<div class="empty-state">Register first to receive your referral QR.</div>'; return;} wrap.innerHTML=`<section class="qr-card"><h3>Your invite QR</h3><div class="qrbox">REF-${p.qr}</div><p>Reward rule: friend registers through your QR, places the first online order, pays, and the order is completed. Then you receive €0.50 Langar Credit.</p></section>`; }
...
function renderInbox(){ const list=[...cards().filter(c=>c.status==='active').map(c=>({title:c.title,body:c.body,type:'card',createdAt:c.createdAt})), ...inbox()]; const box=$('#inboxList'); box.innerHTML=list.map(x=>`<article class="inbox-item"><b>${x.title}</b><p>${x.body}</p><small>${new Date(x.createdAt).toLocaleString()}</small></article>`).join('')||'<p class="muted">Inbox is empty.</p>'; }
...
function setupEvents(){ attachNav(); $('#langBtn').onclick=()=>setLang(state.lang==='hr'?'en':'hr'); $$('.segmented [data-order-type]').forEach(b=>b.onclick=()=>{$$('.segmented button').forEach(x=>x.classList.remove('active')); b.classList.add('active'); state.orderType=b.dataset.orderType;}); $('#closeModal').onclick=()=>$('#modal').classList.add('hidden'); $('#modal').onclick=e=>{if(e.target.id==='modal')$('#modal').classList.add('hidden')}; $('#inboxBtn').onclick=()=>{renderInbox();$('#inboxPanel').classList.remove('hidden')}; $('#closeInbox').onclick=()=>$('#inboxPanel').classList.add('hidden'); $('#closePopup').onclick=$('#popupLater').onclick=()=>{ $('#welcomePopup').classList.add('hidden'); localStorage.langar_popup_closed='1';}; $('#clubForm').onsubmit=e=>{ e.preventDefault(); const data=Object.fromEntries(new FormData(e.target).entries()); const p={...data, id:uid('CUST'), qr:'LNG-'+Math.floor(100000+Math.random()*900000), credit:0, orders:0, visits:0, referrals:0, createdAt:new Date().toISOString()}; LS.set('langar_profile',p); const free=createCard('welcome','Free Espresso Card','One-time free espresso welcome gift. Show this card and let staff scan it.', 'free'); LS.set('langar_cards',[free,...cards()]); addInbox({id:uid('msg'),title:'Welcome to Langar Club',body:'Your free espresso card is ready in Rewards.',unread:true,createdAt:new Date().toISOString()}); $('#clubResult').classList.remove('hidden'); $('#clubResult').innerHTML=`<h3>${p.firstName} — Langar Club</h3><div class="qrbox">${p.qr}</div><p>Your personal QR is ready. Welcome espresso card added.</p>`; e.target.reset(); renderAll();}; const reservationDate=$('#reservationDate'); if(reservationDate) reservationDate.onchange=()=>{ $('#reservationTime').value=''; renderReservationCalendar();}; $('#reservationForm').onsubmit=e=>{e.preventDefault(); const data=Object.fromEntries(new FormData(e.target).entries()); if(!data.time){ alert(state.lang==='hr'?'Molimo odaberite slobodan zeleni termin.':'Please choose an available green time slot.'); return; } if(isReservationSlotBooked(data.date,data.time)){ alert(state.lang==='hr'?'Ovaj termin je već zauzet. Odaberite drugi zeleni termin.':'This time slot is already booked. Please choose another green slot.'); renderReservationCalendar(); return; } const res=LS.get('langar_reservations',[]); res.unshift({id:uid('RES'),status:'pending',createdAt:new Date().toISOString(),...data}); LS.set('langar_reservations',res); alert(state.lang==='hr'?'Rezervacija je poslana. Termin je sada zauzet dok ga admin ne otkaže.':'Reservation sent. This time slot is now blocked unless admin cancels it.'); e.target.reset(); $('#reservationTime').value=''; renderReservationCalendar();}; $('#submitOrder').onclick=()=>{ if(!state.cart.length)return alert(T[state.lang].emptyCart); const total=state.cart.reduce((s,it)=>s+priceNum(it.price)*it.qty,0); const orders=LS.get('langar_orders_v3',[]); orders.unshift({id:uid('ORD'),status:'new',paid:false,type:state.orderType,name:$('#orderName').value,phone:$('#orderPhone').value,address:$('#orderAddress').value,note:$('#orderNote').value,items:state.cart,total:+total.toFixed(2),createdAt:new Date().toISOString()}); LS.set('langar_orders_v3',orders); state.cart=[]; renderCart(); alert(T[state.lang].orderSaved);}; if(!localStorage.langar_popup_closed) setTimeout(()=>$('#welcomePopup').classList.remove('hidden'),800); }
