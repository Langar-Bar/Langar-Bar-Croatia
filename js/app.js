...
function renderHomeMarketing(){ const wrap=$('#homeMarketingHub'); if(!wrap) return; const popular=rankedItems(); const daily=LS.get('langar_daily_surprise',{title:{hr:'Današnje iznenađenje',en:'Today’s Surprise'},body:{hr:'Pridružite se Langar Clubu i pratite posebne ponude za otvorenje.',en:'Join Langar Club and watch for opening offers.'}}); const poll=LS.get('langar_poll',{question:{hr:'Koji okus želite sljedeći?',en:'Which flavor should come next?'},options:['Pistachio Protein','Mango Ice Cream','Spicy Taco','Chocolate Dessert'],votes:{}}); const feedback=LS.get('langar_feedback',[]).filter(f=>+f.rating>=4).slice(0,3); const recent=lastCompletedOrRecentOrder(); const nextEvents=eventsList().slice(0,3);
  ...
  <div class="experience-grid">
    ...
    <article class="experience-card" data-go="feedback"><span>⭐</span><h3>${state.lang==='hr'?'Ostavite feedback':'Leave feedback'}</h3><p>${state.lang==='hr'?'Pozitivne recenzije mogu biti javne; problemi idu samo adminu.':'Positive reviews may be public; problems go to admin only.'}</p></article>
  </div>
  <section class="section-head compact-head"><h2>${state.lang==='hr'?'Event Calendar':'Event Calendar'}</h2><p>${state.lang==='hr'?'Prijavite interes za nadolazeće večeri i radionice.':'Register interest for upcoming nights and workshops.'}</p></section>
  <div class="mini-events">${nextEvents.map(e=>`<article data-go="events"><span>${e.icon||'✦'}</span><b>${textOf(e.title)}</b><small>${e.date||''}</small></article>`).join('')}</div>
  <section class="section-head compact-head"><h2>${state.lang==='hr'?'Popularno i najviše lajkano':'Popular & Most Liked'}</h2><p>${state.lang==='hr'?'Računa se prema narudžbama, lajkovima i istaknutim artiklima.':'Calculated from orders, likes and featured items.'}</p></section><div class="popular-strip" id="popularStrip"></div>
  ...
  const orderBtn=$('#orderAgainBtn'); if(orderBtn) orderBtn.onclick=()=>addOrderAgain(recent);
  const pollBox=$('#homePollOptions'); if(pollBox){ pollBox.innerHTML=poll.options.map(o=>`<button class="poll-btn">${o} <small>${poll.votes[o]||0}</small></button>`).join(''); $$('#homePollOptions .poll-btn').forEach((b,i)=>b.onclick=()=>{ const opt=poll.options[i]; poll.votes[opt]=(poll.votes[opt]||0)+1; LS.set('langar_poll',poll); renderHomeMarketing(); }); }
  const strip=$('#popularStrip'); if(strip){ strip.innerHTML=popular.map(i=>`<article class="popular-card"><b>${itemName(i)}</b><span>${i.price}</span><small>${i.orderCount?`${i.orderCount} orders · `:''}${i.liked?'♥ liked':''}</small></article>`).join(''); }
  attachNav();
}
...
function defaultEvents(){ return [
  {id:'EVT-COFFEE-TALK', icon:'☕', title:{hr:'Coffee Talk Night',en:'Coffee Talk Night'}, date:'Opening season', body:{hr:'Kratka večer za goste: espresso, cappuccino, voda uz kavu, crema i kako prepoznati dobru kavu.',en:'A short guest evening: espresso, cappuccino, water with coffee, crema and how to recognize good coffee.'}},
  {id:'EVT-TACO-NIGHT', icon:'🌮', title:{hr:'Taco & Tapas Night',en:'Taco & Tapas Night'}, date:'Coming soon', body:{hr:'Druženje uz tacose, tapas zalogaje i posebne kombinacije pića.',en:'A social evening with tacos, tapas bites and special drink pairings.'}},
  {id:'EVT-DESSERT-PAIRING', icon:'🧁', title:{hr:'Dessert & Coffee Pairing',en:'Dessert & Coffee Pairing'}, date:'Coming soon', body:{hr:'Kako spojiti desert, espresso, cappuccino i hladne napitke u bolji doživljaj.',en:'How to pair dessert, espresso, cappuccino and cold drinks for a better experience.'}},
  {id:'EVT-PROTEIN-DAY', icon:'💪', title:{hr:'Protein Drink Day',en:'Protein Drink Day'}, date:'Coming soon', body:{hr:'Dan za sportaše i aktivne goste: protein napitci, lagani snack i posebne ponude.',en:'A day for athletes and active guests: protein drinks, light snacks and special offers.'}},
  {id:'EVT-BARISTA', icon:'🎓', title:{hr:'Barista Basics Workshop',en:'Barista Basics Workshop'}, date:'Coming soon', body:{hr:'Uvod u espresso, mljevenje, mliječnu pjenu i higijenu aparata.',en:'Introduction to espresso, grinding, milk foam and machine hygiene.'}}
]; }
function eventsList(){ return LS.get('langar_events', defaultEvents()); }
function renderEventCalendar(){ const box=$('#eventCalendarView'); if(!box) return; box.innerHTML=eventsList().map(ev=>`<article class="event-card"><span>${ev.icon||'✦'}</span><div><h3>${textOf(ev.title)}</h3><small>${ev.date||''}</small><p>${textOf(ev.body)}</p><button class="secondary event-interest" data-event="${ev.id}">${state.lang==='hr'?'Zanima me':'I’m interested'}</button></div></article>`).join(''); $$('.event-interest').forEach(btn=>btn.onclick=()=>{ const ev=eventsList().find(e=>e.id===btn.dataset.event); const p=profile(); const interests=LS.get('langar_event_interests',[]); interests.unshift({id:uid('EI'), eventId:ev.id, eventTitle:textOf(ev.title,'en'), customerId:p?.id||null, name:p?.firstName||'', createdAt:new Date().toISOString()}); LS.set('langar_event_interests', interests); addInbox({id:uid('msg'), type:'message', title: state.lang==='hr'?'Interes za event spremljen':'Event interest saved', body: state.lang==='hr'?`Spremili smo vaš interes za: ${textOf(ev.title,'hr')}. Kada termin bude potvrđen, obavijest dolazi u Inbox.`:`We saved your interest for: ${textOf(ev.title,'en')}. When the date is confirmed, you will receive an Inbox notice.`, unread:true, createdAt:new Date().toISOString()}); renderInboxBadge(); alert(state.lang==='hr'?'Interes je spremljen u Inbox.':'Interest saved in Inbox.'); }); }
...
function renderAll(){ renderCategoryTabs(); renderMenu(); renderOrderCategoryTabs(); renderOrderMenu(); renderCart(); renderDashboard(); renderHomeMarketing(); renderPublicFeedback(); renderAppQr(); renderInboxBadge(); renderReservationCalendar(); renderEventCalendar(); }
