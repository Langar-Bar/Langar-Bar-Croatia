...
function updateDialog(remote,force=false){
 let box=document.getElementById('langarUpdateV604');if(box)return;
 document.getElementById('langarUpdateV601')?.remove();
 const L=getLang();box=document.createElement('div');box.id='langarUpdateV604';box.className='update-v604';
 box.innerHTML=`<div><h3>${L==='hr'?'Dostupna je nova verzija':'A new version is available'}</h3><p>${L==='hr'?'Ažurirajte aplikaciju kako biste dobili najnovija poboljšanja.':'Update the app to receive the latest improvements.'}</p><div class="update-actions-v604"><button id="langarUpdateNowV604" class="update-primary-v604">${L==='hr'?'Ažuriraj sada':'Update now'}</button>${force?'':`<button id="langarUpdateLaterV604" class="update-secondary-v604">${L==='hr'?'Kasnije':'Later'}</button>`}</div></div>`;
 document.body.appendChild(box);
 box.querySelector('#langarUpdateLaterV604')?.addEventListener('click',()=>box.remove());
 box.querySelector('#langarUpdateNowV604').onclick=hardUpdate;
}
async function hardUpdate(){
 const box=document.getElementById('langarUpdateV604');const btn=document.getElementById('langarUpdateNowV604');
 if(btn){btn.disabled=true;btn.textContent=getLang()==='hr'?'Ažuriranje…':'Updating…'}
 const targetVersion=window.__langarRemoteVersion||VERSION;
 try{
  sessionStorage.setItem('langar_update_in_progress',String(Date.now()));
  localStorage.setItem('langar_applied_version',targetVersion);
  if('serviceWorker'in navigator){
   const regs=await navigator.serviceWorker.getRegistrations();
   for(const r of regs){try{await r.update();if(r.waiting)r.waiting.postMessage({type:'SKIP_WAITING'})}catch(_){}}
  }
  if('caches'in window){for(const k of await caches.keys())await caches.delete(k)}
 }catch(_){ }
 box?.remove();
 const url=new URL(location.href);url.searchParams.set('v','604');url.searchParams.set('_u',Date.now());
 setTimeout(()=>location.replace(url.toString()),150);
}
async function checkVersion(){
 ...
 try{
  ...
  const applied=localStorage.getItem('langar_applied_version')||'';
  const justUpdated=Number(sessionStorage.getItem('langar_update_in_progress')||0);
  const grace=justUpdated&&Date.now()-justUpdated<120000;
  if(remote&&compareVersions(remote,VERSION)>0&&!grace&&applied!==remote)updateDialog(remote,!!j.force_update);
  else{
   document.getElementById('langarUpdateV604')?.remove();
   if(!remote||compareVersions(remote,VERSION)<=0){localStorage.setItem('langar_applied_version',remote||VERSION);sessionStorage.removeItem('langar_update_in_progress')}
  }
